...
class _bonafideletterState extends State<bonafideletter> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      stream: FirebaseFirestore.instance.collection('bonafideletter').snapshots(),
      ...
      return const SizedBox(); // You can return an empty widget or handle this case as per your requirement.
            ...
            return TextButton(
              ...
              context,
              MaterialPageRoute(
                builder: (context) => bonafidedetails(m: data, d: document.id),
              ),
    ...
    // if(snapshot.hasData && snapshot.data!.docs.isNotEmpty) {               return Center(child: const Text('No tasks found'));
      ...
      appBar: AppBar(centerTitle:true,title: const Text('Bonafide Letter',style: TextStyle(color: Colors.white,fontSize: 40),),backgroundColor: const Color.fromARGB(255, 223, 130, 161),leading: IconButton(icon: const Icon(Icons.exit_to_app),onPressed: () {Navigator.pop(context);
