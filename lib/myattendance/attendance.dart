...
class _attendanceState extends State<attendance> {
  ...
  for (QueryDocumentSnapshot eventDoc in eventSnapshot.docs) {
    ...
    if (eventName != null && eventTime != null) {
      ...
      DateTime onlyDate=DateTime(eventDateTime.year,eventDateTime.month,eventDateTime.day);
...
DateTime now=DateTime.now();
 ...
 final TextEditingController _eventController = TextEditingController();
  ...
  QuerySnapshot querySnapshot = await firestore.collection('events').where('event', isEqualTo: a).get();

  for (QueryDocumentSnapshot docSnapshot in querySnapshot.docs) {
    await docSnapshot.reference.delete();
  }
}
  Future Submit (String a,DateTime b)
    async { 
FirebaseFirestore firestore=FirebaseFirestore.instance;
...
Future<void> _addEvent(String eventName) async {
  await Submit(eventName, now);
   Map<DateTime, List<Event>> eventsMap = await update(); // Fetch updated data from Firestore
  setState(() {
    selectedEvents = eventsMap;
  ...
  void initState() {
   ...
   Future<void> _updateSelectedEventsFromFirestore() async {
    Map<DateTime, List<Event>> eventsMap = await update(); // Fetch data from Firestore
    setState(() {
      selectedEvents = eventsMap;
    });
  }
  ...
  List<Event> _getEventsfromDay(DateTime date) {     //        print(selectedEvents[date]);
  ...
  itemBuilder: (context, index) {
    ...
    return Container(
      ...
      floatingActionButton: FloatingActionButton.extended(
       onPressed: () { if (isSameDay(focusedDay,now)) { showDialog(
          context: context,
